...
const FolderWickelfalz6seiten_RecordService = require("../../../../service/v1/FolderWickelfalz6seiten_Record")
const DigiPrint_MaterialService = require("../../../../service/v1/DigiPrint_Material")
const FolderWickelfalz6seiten_EndFormatService = require("../../../../service/v1/FolderWickelfalz6seiten_EndFormat")
const OrderService = require("../../../../service/v1/Order")
const FolderWickelfalz6seiten_RecordView = require("../../../../view/client/v1/FolderWickelfalz6seiten_Record")
...
module.exports = new (class FolderWickelfalz6seiten_RecordController extends BaseController {
  async createOrder(req, res) {
    try {
      const { doubleSided, count, colorful, isCellophane, cellophaneType, materialId, endFormatId } = req.body
      const { width, height, _doc } = await FolderWickelfalz6seiten_EndFormatService.findOneByCondition({ _id: endFormatId })
      ...
      const Price = await FolderWickelfalz6seiten_RecordService.calculatorPrice(reqObject, DigiPrint_Materials)
      const Object = {
        count: count,
      ...
      const CreatedFlyerOrder = await FolderWickelfalz6seiten_RecordService.createObject(Object)
      ...
      const OrderItem = await OrderService.createObject({
        tableName: "FolderWickelfalz6seiten_Record",
        tableId: CreatedFlyerOrder._id,
        productName: `Folder Wickelfalz 6-seitig ${_doc.key} ${DigiPrint_Materials.key} ${
      ...
      return res.status(200).json({
        data: FolderWickelfalz6seiten_RecordView.transform(CreatedFlyerOrder)
      })
    } catch (err) {
    ...
    try {
      ...
      const MatrialWithPrice = await Promise.all(
        DigiPrint_Materials.map(async (Material) => {
          const Price = await FolderWickelfalz6seiten_RecordService.calculatorPrice(reqObject, Material)

...
const FolderZfalz8Seiten_RecordService = require("../../../../service/v1/FolderZfalz8Seiten_Record")
...
const FolderZfalz8Seiten_RecordView = require("../../../../view/client/v1/FolderZfalz8Seiten_Record")
...
module.exports = new (class FolderZfalz8Seiten_RecordController extends BaseController {
  async createOrder(req, res) {
    try {
      const { doubleSided, count, colorful, isCellophane, cellophaneType, materialId, endFormatId } = req.body
      const { width, height, _doc } = await FolderZfalz8Seiten_EndFormatService.findOneByCondition({ _id: endFormatId })
      ...
      const Price = await FolderZfalz8Seiten_RecordService.calculatorPrice(reqObject, DigiPrint_Materials)
      const Object = {
        count: count,
      ...
      const CreatedFlyerOrder = await FolderZfalz8Seiten_RecordService.createObject(Object)
      ...
      const OrderItem = await OrderService.createObject({
        tableName: "FolderZfalz8Seiten_Record",
        tableId: CreatedFlyerOrder._id,
        productName: `Folder Z-falz 8-seitig ${_doc.key} ${DigiPrint_Materials.key} ${String(doubleSided) === "true" ? "beidseitig" : "einseitig"} ${
      ...
      return res.status(200).json({
        data: FolderZfalz8Seiten_RecordView.transform(CreatedFlyerOrder)
      })
    } catch (err) {
    ...
    try {
      ...
      const MatrialWithPrice = await Promise.all(
        DigiPrint_Materials.map(async (Material) => {
          const Price = await FolderZfalz8Seiten_RecordService.calculatorPrice(reqObject, Material)
          return { ...Material._doc, price: Price }
        })
      )

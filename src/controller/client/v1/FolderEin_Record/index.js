...
const FolderEin_RecordService = require("../../../../service/v1/FolderEin_Record")
const DigiPrint_MaterialService = require("../../../../service/v1/DigiPrint_Material")
const FolderEin_EndFormatService = require("../../../../service/v1/FolderEin_EndFormat")
const OrderService = require("../../../../service/v1/Order")
const FolderEin_RecordView = require("../../../../view/client/v1/FolderEin_Record")
...
module.exports = new (class FolderEin_RecordController extends BaseController {
  async createOrder(req, res) {
    try {
      const { doubleSided, count, colorful, isCellophane, cellophaneType, materialId, endFormatId } = req.body
      const { width, height, _doc } = await FolderEin_EndFormatService.findOneByCondition({ _id: endFormatId })
      ...
      const Price = await FolderEin_RecordService.calculatorPrice(reqObject, DigiPrint_Materials)
      ...
      const CreatedFlyerOrder = await FolderEin_RecordService.createObject(Object)
      ...
      const OrderItem = await OrderService.createObject({
        tableName: "FolderEin_Record",
        tableId: CreatedFlyerOrder._id,
        productName: `Folder Einfachfalz 4-seitig ${_doc.key} ${DigiPrint_Materials.key} ${
      ...
      return res.status(200).json({
        data: FolderEin_RecordView.transform(CreatedFlyerOrder)
      })
    } catch (err) {
    ...
    try {
      ...
      const MatrialWithPrice = await Promise.all(
        DigiPrint_Materials.map(async (Material) => {
          const Price = await FolderEin_RecordService.calculatorPrice(reqObject, Material)

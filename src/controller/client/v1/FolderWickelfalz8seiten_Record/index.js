...
const FolderWickelfalz8seiten_RecordService = require("../../../../service/v1/FolderWickelfalz8seiten_Record")
...
const FolderWickelfalz8seiten_RecordView = require("../../../../view/client/v1/FolderWickelfalz8seiten_Record")
...
module.exports = new (class FolderWickelfalz8seiten_RecordController extends BaseController {
  async createOrder(req, res) {
    try {
      const { doubleSided, count, colorful, isCellophane, cellophaneType, materialId, endFormatId } = req.body
      const { width, height, _doc } = await FolderWickelfalz8seiten_EndFormatService.findOneByCondition({ _id: endFormatId })
      ...
      const Price = await FolderWickelfalz8seiten_RecordService.calculatorPrice(reqObject, DigiPrint_Materials)
      const Object = {
        count: count,
      ...
      const CreatedFlyerOrder = await FolderWickelfalz8seiten_RecordService.createObject(Object)
      ...
      const OrderItem = await OrderService.createObject({
        tableName: "FolderWickelfalz8seiten_Record",
        tableId: CreatedFlyerOrder._id,
        productName: `Folder Wickelfalz 8-seitig ${_doc.key} ${DigiPrint_Materials.key} ${
      ...
      return res.status(200).json({
        data: FolderWickelfalz8seiten_RecordView.transform(CreatedFlyerOrder)
      })
    } catch (err) {
    ...
    try {
      ...
      const MatrialWithPrice = await Promise.all(
        DigiPrint_Materials.map(async (Material) => {
          const Price = await FolderWickelfalz8seiten_RecordService.calculatorPrice(reqObject, Material)

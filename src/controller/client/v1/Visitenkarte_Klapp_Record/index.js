...
const Visitenkarte_Klapp_RecordService = require("../../../../service/v1/Visitenkarte_Klapp_Record")
const DigiPrint_MaterialService = require("../../../../service/v1/DigiPrint_Material")
const OrderService = require("../../../../service/v1/Order")
const Visitenkarte_Klapp_RecordView = require("../../../../view/client/v1/Visitenkarte_Klapp_Record")
...
module.exports = new (class Visitenkarte_Klapp_RecordController extends BaseController {
  async createOrder(req, res) {
    try {
      ...
      const Price = await Visitenkarte_Klapp_RecordService.calculatorPrice(req, DigiPrint_Materials)
      const Object = {
        count: count,
      ...
      const CreatedFlyerOrder = await Visitenkarte_Klapp_RecordService.createObject(Object)
      ...
      const OrderItem = await OrderService.createObject({
        tableName: "Visitenkarte_Klapp_Record",
        tableId: CreatedFlyerOrder._id,
        productName: `Visitenkarte Klapp ${width}x${height}mm ${DigiPrint_Materials.key} ${
      ...
      return res.status(200).json({
        data: Visitenkarte_Klapp_RecordView.transform(CreatedFlyerOrder)
      })
    } catch (err) {
    ...
    try {
      ...
      const MatrialWithPrice = await Promise.all(
        DigiPrint_Materials.map(async (Material) => {
          const Price = await Visitenkarte_Klapp_RecordService.calculatorPrice(req, Material)

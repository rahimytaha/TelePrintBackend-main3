...
module.exports = new (class Lesezeichen_MaterialController extends BaseController {
  async getAll(req, res) {
    try {
      const { isCellophane } = req.body
      const Lesezeichen_Materials = await Lesezeichen_MaterialService.findAllAndSort(
        {
      ...
      if (!Lesezeichen_Materials || !Lesezeichen_Materials.length) {
      ...
      const MatrialWithPrice = await Promise.all(
        Lesezeichen_Materials.map(async (Material) => {
